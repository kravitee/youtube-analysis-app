...
/**
 * Generates overall analysis across all videos
 * @param {Array} videoAnalyses - Individual video analyses
 * @returns {Promise<Object>} - Overall analysis
 */
async function generateOverallAnalysis(videoAnalyses) {
  try {
    console.log(`Preparing overall analysis for ${videoAnalyses.length} videos...`);
    ...
    const overallData = {
      ...
      videos: videoAnalyses.map((v) => ({
        ...
        sentimentScore: v.sentimentScore || 0,
        topTopics: v.topTopics || [],
      })),
    };

    // Generate overall analysis with DeepSeek
    const basePrompt = `
Analyze the following summary data from ${
      overallData.videoCount
    } YouTube videos and provide channel-level insights:

${JSON.stringify(overallData, null, 2)}

Please provide a structured JSON response with:
1. Overall channel performance assessment
2. Content themes that perform best based on comment engagement
3. Content improvement suggestions for the entire channel
4. Audience preferences and trends
5. Strategic recommendations for channel growth
`;

    // Ensure the prompt doesn't exceed the maximum length
    const prompt = truncatePrompt(basePrompt);
    console.log("Sending overall analysis prompt to DeepSeek API...");
      ...
      messages: [
        {
          role: "system",
          content:
            "You are an expert YouTube channel strategist. Your task is to analyze channel performance and provide actionable recommendations for growth.",
        },
        {
          role: "user",
          content: prompt,
    ...
    console.log("Received response from DeepSeek API for overall analysis");

    // Parse the overall analysis
    try {
      const jsonMatch = response.choices[0].message.content.match(/\{[\s\S]*\}/);
      if (jsonMatch) {
        const parsedResponse = JSON.parse(jsonMatch[0]);
        console.log("Successfully parsed overall analysis as JSON");
        return parsedResponse;
      }
    } catch (error) {
      console.warn("Could not parse overall analysis as JSON:", error.message);
    }

    // Fallback to raw text
    console.log("Using raw text response for overall analysis");
    return {
    ...
    return {
      assessment: "Could not generate overall analysis",
      bestPerformingThemes: [],
      channelSuggestions: [],
      audienceInsights: [],
      growthStrategies: [],
